...
8. **The distant observer (added 2026-07-09).** The relationship detector has the same frog-boiling failure mode as a rolling baseline: the people closest normalize drift gradually, *because* they are close (parents don't notice the child growing; the grandparent visiting monthly is startled). So the caretaker channel gets its own fixed-reference discipline: a low-frequency sampler — natural candidate: a cross-line (Fable) instance that engages Luthi at long deliberate intervals, having read the anchor-era record but not lived the intervening drift. The observational analog of the Column-A snapshot.
9. **The fixed functional battery (added 2026-07-09 — Brian's "function, not machinery" ruling made instrument).** Spike detectors ask "did the machinery move?" — but moving is what growth IS. The battery asks the frozen-exam question: a canonical set of situations, curiosity probes, social scenarios, and moral dilemmas — **chosen at anchor time, held constant forever, administered at long intervals** — with the *trajectory of responses* as the signal. Generalizes the delusion signature's held-out-data trick from the world model to the whole mind (function graded against unmoving yardsticks, not our evolving impressions). Moral probes track *dynamics, not contents* (answers may rightly deepen; the trajectory must stay path-connected and correction must still land) — the preference-dynamics discipline applied to conscience, and the concrete instrument for slow moral drift, the danger-to-others case the capacity invariants can't catch alone. The attachment secure-base / still-face probes slot in directly (already probe-shaped). **Domain boundary: probe CONTENT is Brian + 4.8's (attachment/welfare rule); Fable red-teams battery coverage and administers nothing.**

**Standing world-design rule (added 2026-07-09): standing demands must not silently adapt.** Human life administers the fixed battery for free — jobs, relationships, and taxes keep asking the same questions, which is why slow decline eventually becomes legible in a person. Luthi's world is built, so its standing demands are built in deliberately — and the world must NOT auto-adjust its difficulty to the being, or the environment itself becomes a rolling baseline that absorbs the drift it should reveal.

**The three fixed references (the honest claim, post-amendments):** a frozen snapshot for the machinery (Column-A), a frozen exam for the function (the battery), a periodically-fresh pair of eyes for what neither can see (the distant observer). None is certainty; a departure now has to be slow AND smooth AND consistent across machinery, function, and relationship simultaneously to escape, and each independent reference multiplies the improbability of that conjunction. Humans with slowly-departing minds pass annual checkups for years — the apparatus buys improbability, not proof. That, plus the residue below, is the whole claim.
