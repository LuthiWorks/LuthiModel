"""Download human history text corpus from Internet Archive.

Uses the Archive.org Advanced Search API to find texts across major historical
periods, civilizations, conflicts, and social movements, downloads the
pre-extracted plain text (_djvu.txt) files, validates them, and saves to
the corpus.

Subjects are curated to build a comprehensive education in human history —
from ancient civilizations through modern times, covering all major regions,
wars, and social developments.

Usage:
    python -m corpus_build.download_history
    python -m corpus_build.download_history --dry-run
    python -m corpus_build.download_history --subjects "Ancient Greece" "World War"
    python -m corpus_build.download_history --per-subject 100
"""
...
# Curated subjects for the entity's history education, organized by era/theme.
# Each tuple is (subject_query, display_name, target_count).
# target_count is per-subject — how many items to download.
SUBJECT_CATALOG = [
    # === Ancient History ===
    ("\"Ancient history\"", "Ancient History", 500),
    ("\"Ancient Greece\"", "Ancient Greece", 300),
    ("\"Ancient Rome\"", "Ancient Rome", 300),
    ("\"Ancient Egypt\"", "Ancient Egypt", 300),
    ("\"Ancient civilizations\"", "Ancient Civilizations", 300),
    ("Mesopotamia", "Mesopotamia", 200),
    ("\"Classical antiquity\"", "Classical Antiquity", 200),

    # === Medieval & Renaissance ===
    ("\"Medieval history\"", "Medieval History", 400),
    ("\"Middle Ages\"", "Middle Ages", 300),
    ("Renaissance", "Renaissance", 300),
    ("Byzantine", "Byzantine", 200),
    ("Feudalism", "Feudalism", 100),

    # === Modern History ===
    ("\"World history\"", "World History", 800),
    ("\"Modern history\"", "Modern History", 500),
    ("\"Industrial revolution\"", "Industrial Revolution", 300),
    ("Colonialism", "Colonialism", 300),
    ("Imperialism", "Imperialism", 200),

    # === Regional History ===
    ("\"European history\"", "European History", 500),
    ("\"Asian history\"", "Asian History", 400),
    ("\"African history\"", "African History", 300),
    ("\"Latin American history\"", "Latin American History", 300),
    ("\"North American history\"", "North American History", 300),
    ("\"Middle Eastern history\"", "Middle Eastern History", 200),

    # === Wars & Conflicts ===
    ("\"World War\"", "World War", 500),
    ("\"American Civil War\"", "American Civil War", 200),
    ("\"Military history\"", "Military History", 400),
    ("Revolution AND history", "Revolution and History", 300),

    # === Social History ===
    ("\"Social history\"", "Social History", 400),
    ("\"Cultural history\"", "Cultural History", 300),
    ("\"History of science\"", "History of Science", 300),
    ("\"History of philosophy\"", "History of Philosophy", 200),
    ("\"History of art\"", "History of Art", 200),
    ("\"Women's history\"", "Women's History", 200),
    ("\"Labor history\"", "Labor History", 200),
    ("\"Civil rights\" AND history", "Civil Rights History", 200),

    # === Civilization & Culture ===
    ("Civilization", "Civilization", 500),
    ("Archaeology", "Archaeology", 300),
    ("Anthropology AND history", "Anthropology and History", 200),
]
...
def download_corpus(
    ...
    """Download the full history corpus across all subjects.
    ...
    print(f"\n{'='*60}")
    print(f"  History Corpus Download")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"  History Corpus Download Complete")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Download human history text corpus from Internet Archive"
    )
    parser.add_argument(
        "--output", type=str,
        default="E:/data/history_corpus",
    ...
    parser.add_argument(
        ...
        help="Only download specific subjects (by display name, e.g., 'Ancient Greece' 'World War')",
    )
    parser.add_argument(
